// gop/gop-gmm.cc
// modified to include the goodness of nasality feature
...
BaseFloat GmmGop::ComputeGonDenominViterbi(DecodableAmDiagGmmScaled &decodable,
                                          int32 phone_l, int32 phone, int32 phone_r) {
  KALDI_ASSERT(ctx_dep_.ContextWidth() == 3);
  KALDI_ASSERT(ctx_dep_.CentralPosition() == 1);
  std::vector<int32> phoneseq(3);
  ...
  //determine cognate phoneme
  // check for N cognates (T, D, DH, S, Z, ZH, TH)
  int32 Td = (phone - 287) % 346;
  int32 Dd = (phone - 115) % 346;
  int32 DHd = (phone - 119) % 346;
  int32 Sd = (phone - 279) % 346;
  int32 Zd = (phone - 339) % 346;
  int32 ZHd = (phone - 343) % 346;
  int32 THd = (phone - 291) % 346;

  int32 Nstart = 231;

  // check for M cognates (P, B, F)
  int32 Pd = (phone - 271) % 346;
  int32 Bd = (phone - 107) % 346;
  int32 Fd = (phone - 171) % 346;

  int32 Mstart = 227;

  // check for NG cognates (K, G, H)
  int32 Kd = (phone - 219) % 346;
  int32 Gd = (phone - 175) % 346;
  int32 HHd = (phone - 179) % 346;

  int32 NGstart = 235;

  if(Td == 0)
  {
    phoneseq[1] = Nstart + Td;
  } else if(Dd == 0)
  {
    phoneseq[1] = Nstart + Dd;
  } else if(DHd == 0)
  {
    phoneseq[1] = Nstart + DHd;
  } else if(Sd == 0)
  {
    phoneseq[1] = Nstart + Sd;
  } else if(Zd == 0)
  {
    phoneseq[1] = Nstart + Zd;
  } else if(ZHd == 0)
  {
    phoneseq[1] = Nstart + ZHd;
  } else if(THd == 0)
  {
    phoneseq[1] = Nstart + THd;
  } else if(Pd == 0)
  {
    phoneseq[1] = Mstart + Pd;
  } else if(Bd == 0)
  {
    phoneseq[1] = Mstart + Bd;
  } else if(Fd == 0)
  {
    phoneseq[1] = Mstart + Fd;
  } else if(Kd == 0)
  {
    phoneseq[1] = NGstart + Kd;
  } else if(Gd  == 0)
  {
    phoneseq[1] = NGstart + Gd;
  } else if(HHd  == 0)
  {
    phoneseq[1] = NGstart + HHd;
  } else {
    //it is not a cognate of any of the three nasals, so the denominator will be identical to the numerator and the answer will be zero.
    phoneseq[1] = phone;
  }
  ...
  // all possible conditions lead to phoneseq[1] being assigned
...
void GmmGop::Compute(const Matrix<BaseFloat> &feats,
  ...
  for (MatrixIndexT i = 0; i < split.size(); i++) {
    ...
    BaseFloat gon_denominator = ComputeGonDenominViterbi(split_decodable, phone_l, phone, phone_r);
    gop_result_(i) = (gop_numerator - gon_denominator) / split[i].size();
    phones_[i] = phone;
